...
translator = Translator(service_urls=['translate.googleapis.com'], timeout=10)
...
def translate_text(text, dest_lang):
    ...
    for i in range(retries):
        try:
            ...
            print(f"Timeout occurred: {str(e)}. Retrying... ({i+1}/{retries})")
            time.sleep(2)
        except Exception as e:
...
# @app.route('/translated_docs/<path:filename>')
# def serve_translated_document(filename):
#     try:
#         return send_from_directory(app.config['OUTPUT_FOLDER'], filename)
#     except Exception as e:
#         print(f"Error serving translated document: {str(e)}")
#         return jsonify(status='error', message='Error serving translated document.')
